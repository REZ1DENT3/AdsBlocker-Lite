...
var jsExtended=new(function(){this.expr=new Array();this.expr[":attr"]=function(d,sel,selector){return Array.prototype.slice.call(document.querySelectorAll(sel)).filter(function(element){keys=Array.prototype.slice.call(element.attributes).filter(function(attribute){return(new RegExp(selector)).test(attribute.name)});return keys.length})};this.expr[":removeAll"]=function(d,sel){return d.querySelectorAll(sel)}})();function querySelectorAll(d,selector){if(typeof d=="undefined"||d==null||typeof d.querySelectorAll=="undefined"){return[]}
if(selector.indexOf(':')>=0){for(var expr in jsExtended.expr){if(typeof expr=="undefined")break;regex=new RegExp(expr+'(.*?)$');if(regex.test(selector)){newSelector=selector.match(regex);newSelector[0]=selector.substr(0,selector.indexOf(expr));return jsExtended.expr[expr](d,newSelector[0],newSelector[1])}}}
...
return this}
chrome.storage.local.get({rulesData:[],whitelisted:[]},function(items){rules=[];rulesExpr=[];_rules=[];if(typeof items.rulesData.findKey!='undefined'){_rules=items.rulesData.findKey(hostname)}
...
isNotWhitelist=items.whitelisted.indexOf(hostname)<0;document.addEventListener('DOMSubtreeModified',function(mutationEvent){blocker(mutationEvent.target)},!1)})
...
document.addEventListener("fullscreenchange",function(){onFullScreen(document.fullscreen)},!1);document.addEventListener("mozfullscreenchange",function(){onFullScreen(document.mozFullScreen)},!1);document.addEventListener("webkitfullscreenchange",function(){onFullScreen(document.webkitIsFullScreen)},!1)}
function monitorBlocks(){window.addEventListener("message",function receiveMessage(event){if(event.data.type&&(event.data.type=="blockedWindow")){blockerRulesCount++;chrome.runtime.sendMessage({name:"trackEvent",action:"PopupBlocked",label:hostname})}},!1)}
addScript({textContent:inject.toString()+" inject()"});monitorBlocks()
'use strict';var isOpera=!!window.opera||navigator.userAgent.indexOf(' OPR/')>=0;var isChrome=!!window.chrome&&!isOpera;var player=document.querySelector('#player');function skipVideoAd(){if(document.getElementsByClassName('videoAdUi').length>0){document.getElementsByClassName('video-stream html5-main-video')[0].src='';blockerRulesCount++}}
