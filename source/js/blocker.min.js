...
document.documentElement.appendChild(s)};var blockerCount=0;var blockerRulesCount=0;var timerEps=5;var t1=(new Date()).getTime()-timerEps;var hostname=location.href.getHostname();var rules=new Array();var rulesRegExp=new Array();var isNotWhitelist=!0;const ConstAllSite='\\AllSite\\';if(typeof chrome.tabs!='undefined'){chrome.tabs.query({},function(tabs){for(i=0;i<tabs.length;++i){if(tabs[i].active){if(typeof tabs[i].url.getHostname=='function'){hostname=tabs[i].url.getHostname()}
break}}})}
...
if(rules.length){$rules=$(rules.join(', '));blockerRulesCount+=$rules.length;$rules.adsBlock();if(blockerRulesCount){chrome.runtime.sendMessage({name:"setBadgeText",text:''+blockerRulesCount})}
if(typeof $rules.bund=="function"){$rules.bind("DOMSubtreeModified",function(){$(this).adsBlock()})}}
if(typeof callback=="function"){callback()}
return this};chrome.storage.sync.get({rulesData:[],whitelisted:[]},function(items){rules=[];_rules=[];if(typeof items.rulesData[hostname]!='undefined'){_rules=items.rulesData[hostname]}
