...
document.documentElement.appendChild(s)};jQuery.expr[':'].regex=function(elem,index,match){var matchParams=match[3].split(','),validLabels=/^(data|css):/,attr={method:matchParams[0].match(validLabels)?matchParams[0].split(':')[0]:'attr',property:matchParams.shift().replace(validLabels,'')},regexFlags='ig',regex=new RegExp(matchParams.join('').replace(/^\s+|\s+$/g,''),regexFlags);return regex.test(jQuery(elem)[attr.method](attr.property))};var blockerCount=0;var blockerRulesCount=0;var timerEps=5;var t1=(new Date()).getTime()-timerEps;var hostname=window.document.domain;if(hostname.substr(0,4)=='www.'){hostname=hostname.substr(4)}
var rules=new Array();var isNotWhitelist=!0;const ConstAllSite='___AllSite___';function canBlock(){if(blockerCount>15)
return((new Date()).getTime()-t1)>timerEps;return !0}
function adsBlock(e){$.each(e,function(ind,_e){_e.parentNode.removeChild(_e)})}
...
$.each(qsRules,function(ind,item){if(item.addEventListener){item.addEventListener('DOMSubtreeModified',function(){adsBlock(qsRules)},!1)}})}}
...
function hideOverlayAd(){var overlayAdContainer=document.getElementsByClassName('ad-container ad-container-single-media-element-annotations')[0];if(overlayAdContainer&&overlayAdContainer.style.display!=='none'){overlayAdContainer.style.display='none';blockerRulesCount++}}
