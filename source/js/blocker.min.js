...
document.documentElement.appendChild(s)};var blockerCount=0;var timerEps=5;var t1=(new Date()).getTime()-timerEps;var hostname=location.href.getHostname();var rules=new Array();var rulesRegExp=new Array();var isNotWhitelist=!0;const ConstAllSite='\\AllSite\\';if(typeof chrome.tabs!='undefined'){chrome.tabs.query({},function(tabs){for(i=0;i<tabs.length;++i){if(tabs[i].active){if(typeof tabs[i].url.getHostname=='function'){hostname=tabs[i].url.getHostname()}
break}}})}
function canBlock(){if(blockerCount>15)
...
if(rules.length){$rules=$(rules.join(', '));$rules.adsBlock();if(typeof $rules.bund=="function"){$rules.bind("DOMSubtreeModified",function(){$(this).adsBlock()})}}
if(typeof callback=="function"){callback()}
return this};function reloadRules(items){rules=[];_rules=[];if(typeof items.rulesData[hostname]!='undefined'){_rules=items.rulesData[hostname]}
...
jQuery.each(_rules,function(_key){if(_rules[_key].regExp){}
else{rules.push(_rules[_key].rule)}});isNotWhitelist=items.whitelisted.indexOf(hostname)<0;if(typeof chrome.browserAction=='undefined'){jQuery(document).blocker(function(){jQuery(document).ready(function(){t1=0;jQuery(document).bind("DOMSubtreeModified",function(){jQuery(document).blocker()})})})}}
chrome.storage.sync.get({rulesData:[],whitelisted:[]},function(items){reloadRules(items);if(typeof chrome.tabs!='undefined'){chrome.tabs.onUpdated.addListener(function(tabId,changeInfo,tab){hostname=tab.url.getHostname();reloadRules(items);var text='';if(isNotWhitelist){text+=rules.length}
chrome.browserAction.setBadgeText({text:text,tabId:tabId})})}});function inject(){var originalOpenWndFnKey="originalOpenFunction";var originalWindowOpenFn=window.open,originalCreateElementFn=document.createElement,originalCreateEventFn=document.createEvent,windowsWithNames={};var timeSinceCreateAElement=0;var lastCreatedAElement=null;var fullScreenOpenTime;var parentOrigin=(window.location!=window.parent.location)?document.referrer:document.location;window[originalOpenWndFnKey]=window.open;function newWindowOpenFn(){var openWndArguments=arguments,useOriginalOpenWnd=!0,generatedWindow=null;function blockedWndNotification(openWndArguments){parent.postMessage({type:"blockedWindow",args:JSON.stringify(openWndArguments)},parentOrigin)}
