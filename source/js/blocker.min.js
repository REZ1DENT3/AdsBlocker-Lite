...
var jsExtended=new(function(){this.expr=new Array();this.expr[":attr"]=function(sel,selector){return Array.prototype.slice.call(document.querySelectorAll(sel)).filter(function(element){keys=Array.prototype.slice.call(element.attributes).filter(function(attribute){return(new RegExp(selector)).test(attribute.name)});return keys.length})}})();function querySelectorAll(selector){for(var expr in jsExtended.expr){if(typeof expr=="undefined")break;regex=new RegExp(expr+'(.*?)$');if(regex.test(selector)){newSelector=selector.match(regex);newSelector[0]=selector.substr(0,selector.indexOf(expr));return jsExtended.expr[expr](newSelector[0],newSelector[1])}}
return document.querySelectorAll(selector)};var blockerCount=0;var blockerRulesCount=0;var timerEps=5;var t1=(new Date()).getTime()-timerEps;var hostname=window.document.domain;if(hostname.substr(0,4)=='www.'){hostname=hostname.substr(4)}
