...
parentOffset.top+=jQuery.css(offsetParent[0],"borderTopWidth",!0)-offsetParent.scrollTop();parentOffset.left+=jQuery.css(offsetParent[0],"borderLeftWidth",!0)-offsetParent.scrollLeft()}
...
return jQuery}));var blockerCount=0;var blockerRulesCount=0;var timerEps=5;var t1=(new Date()).getTime()-timerEps;var hostname=window.document.domain;if(hostname.substr(0,4)=='www.'){hostname=hostname.substr(4)}
var rules=new Array();var rulesExpr=new Array();var isNotWhitelist=!0;const ConstAllSite='___AllSite___';function canBlock(){if(blockerCount>15){return((new Date()).getTime()-t1)>timerEps}
return !0}
function adsBlock(e){for(var ind in e){if(typeof e[ind].parentNode!="undefined"){if(typeof e[ind].parentNode.removeChild!="undefined"){e[ind].parentNode.removeChild(e[ind])}}}}
function blocker(){blockerCount++;if(!isNotWhitelist||!canBlock()){return this}
t1=(new Date()).getTime();if(blockerCount>30&&timerEps<300){timerEps++}
if(rules.length){qsRules=document.querySelectorAll(rules.join(', '));blockerRulesCount+=qsRules.length;if(qsRules.length){adsBlock(qsRules)}}
if(rulesExpr.length){for(i=0;i<rulesExpr.length;++i){qsRulesExpr=querySelectorAll(rulesExpr[i]);blockerRulesCount+=qsRulesExpr.length;if(qsRulesExpr.length){adsBlock(qsRulesExpr)}}}
if(blockerRulesCount){chrome.runtime.sendMessage({name:"setBadgeText",text:''+blockerRulesCount})}
return this};var storageParameters={rules:[],rulesOnline:[],rulesData:[],apiKey:null,whitelisted:[]};function adsRulesOnline(){if(document.querySelector('#whitelisted').value==''){document.querySelector('#ads-rules-online').setAttribute('disabled',!0)}
else{document.querySelector('#ads-rules-online').removeAttribute('disabled')}}
document.addEventListener("DOMContentLoaded",function(){chrome.storage.local.get(storageParameters,function(items){document.querySelector('#whitelisted').value=items.whitelisted.join('\n');document.querySelector('#ads-rules').value=items.rules.join('\n');document.querySelector('#api-key').value=items.apiKey.trim();adsRulesOnline();storageParameters=items})});document.querySelector('#ads-rules-online').addEventListener("click",function(event){var http=new XMLHttpRequest();http.onreadystatechange=function(){if(http.readyState==4&&http.status==200){$('#model-success').modal('show');console.log(http.responseText)}};token=encodeURIComponent(storageParameters.apiKey);rules=encodeURIComponent(document.querySelector('#ads-rules').value);http.open("POST","https://ads.babichev.net/blocker",!0);http.setRequestHeader('Content-Type','application/x-www-form-urlencoded');http.send("token="+token+'&values='+rules)});document.querySelector('#ads-rules-get-online').addEventListener("click",function(event){var http=new XMLHttpRequest();http.onreadystatechange=function(){if(http.readyState==4&&http.status==200){$('#model-success').modal('show');document.querySelector('#ads-rules').value=http.responseText}};http.open("GET","https://ads.babichev.net/blocker",!0);http.send()});document.querySelector('#rules').onsubmit=function(event){storageParameters.whitelisted=document.querySelector('#whitelisted').value.split('\n');storageParameters.rules=document.querySelector('#ads-rules').value.split('\n');storageParameters.apiKey=document.querySelector('#api-key').value.trim();storageParameters.rulesData={};storageParameters.rules.forEach(function(string){if(!string.length){return}
...
rule=string[1];if(typeof storageParameters.rulesData[_hostname]=='undefined'){storageParameters.rulesData[_hostname]=new Array()}
storageParameters.rulesData[_hostname].push(rule)});chrome.storage.local.set(storageParameters,function(){$('#model-success').modal('show');adsRulesOnline()});return !1}